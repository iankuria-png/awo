...
| D-026 | 2026-09-25 | **Claude self-checks every board** before publishing: it renders each board locally, screenshots it (interactive states too) and fixes breakage (clipping, overlap, contrast). Fixes cover breakage only, not redesigns. | Owner | Answers T-010. Tool: `scripts/canvas-preview.mjs`. |
